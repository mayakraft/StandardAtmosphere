// international standard atmosphere, valid between sea level and +20,000 m
// robby kraft
...
// one calculation to rule them all
atmosphere atmosphereAtAltitude(double altitude);
// or, calculate each separately
...
double speedOfSoundAtAltitude(double altitude){  // altitude in meters
...
double gravityAtAltitude(double altitude){  // altitude in meters
	return SEA_LEVEL_GRAVITY * pow( EARTH_RADIUS / (EARTH_RADIUS+altitude), 2);
}
double temperatureAtAltitude(double altitude){  // altitude in meters
	if(altitude < 0.0 || altitude > 20000.0)
		return -1;
	else if(altitude < 11000.0)
...
double pressureAtAltitude(double altitude){  // altitude in meters
	if(altitude < 0.0 || altitude > 20000.0)
		return -1;
	else if(altitude < 11000.0)
...
double densityAtAltitude(double altitude){  // altitude in meters
